...
def compute_brute_force_classification(model, image_path, nH=8, nW=8):
    ...
    width = int(raw_image.shape[0]/nW)
    height = int(raw_image.shape[1]/nH)

    window_predictions = np.zeros((nH, nW, 3))
    for i in range(nH):
        for j in range(nW):
            window = raw_image[i*width:(i+1)*width,j*height:(j+1)*height,:]
            window = window.reshape((1,window.shape[0],window.shape[1],3))
            window = normalize_resize_image(window,IMG_SIZE)
            prediction = tf.squeeze(model.predict(window))
            prediction = tf.math.softmax(prediction)
            window_predictions[i,j,:] = prediction
...
def compute_convolutional_KxK_classification(model, image_path):
    ...
    K = conv_model.output_shape[1]
    resized_patch = tf.reshape(resized_patch,(1,299,299,3))
    i_step = conv_model(resized_patch)
    i_step = tf.reshape(i_step,(K**2, 2048))
    convolution, classifier = model.layers
    predictionsKxK = tf.math.softmax(classifier(i_step)).numpy()
...
def compute_and_plot_saliency(model, image_path):
    ...
    with tf.GradientTape() as t:
        ...
        # required.
        t.watch(raw_image)
        prediction = model(tf.reshape(normalize_resize_image(raw_image,IMG_SIZE) ,(1, 299, 299, 3)))
        top_class = tf.argmax(prediction.numpy()[0]).numpy()
        best = tf.reduce_max(prediction)
        gradient = t.gradient(best,raw_image)
        M = tf.reduce_max(gradient,axis = 2)
