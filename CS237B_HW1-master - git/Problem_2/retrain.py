...
def get_bottleneck_dataset(model, img_dir, img_size):
    ...
    for i in range(train_img_gen.samples):
        ...
        bottleneck_x_l.append(model.predict(x_i))
...
def retrain(image_dir):
    ...
    retrain_input = tf.keras.layers.Input(shape= (2048,)) ##?
    retrain_layer = tf.keras.layers.Dense(64)(retrain_input) #128 or different?
    retrain_activation = tf.keras.layers.Dense(3, activation='softmax')(retrain_layer)
    retrain_model = tf.keras.Model(inputs=retrain_input, outputs=retrain_activation, name = "classifier")
    loss = tf.keras.losses.CategoricalCrossentropy(name = 'loss')
    metric = ['accuracy']

    print("retrain model", retrain_model.summary())
    ...
    model.add(tf.keras.layers.Dense(64, name = "classifier"))
    model.add(tf.keras.layers.Dense(3, activation='softmax'))


    print("seq summary", model.summary())
