...
if __name__ == '__main__':
    ...
    for _ in range(15):
        ...
        while not done:
            ...
            gs = goals[scenario_name]
            probs = []
            for goal in gs:
                # mean and variance
                y_est = nn_models[goal](obs).numpy()
                y_est = y_est[0]
                #first two values are the means
                u = y_est[0:2]


                # next 4 values are the cov matrix
                cov = np.reshape(y_est[2:6],(2,2))
                cov_t = np.transpose(cov)
                cov_final = cov@cov_t 
                # prevents nan
                delta = 1e-3
                id_delt = delta*np.eye(2)
                cov_final += delta
                

                # find probability distribution
                mn = multivariate_normal(mean = u,cov = cov_final)
                pp = mn.pdf(action)
                probs.append(pp)
