...
def Q_learning(Q_network, reward_fn, is_terminal_fn, X, U, Xp, gam):
    ...
    def loss():
        ...
        td = reward_fn(X_,U_) + gam*next_Q - reward_fn(X_,U_)*gam*next_Q - Q
        l = tf.reduce_mean(tf.square(td))
    ...
    for _ in tqdm(range(int(1e4))):
        ...
        Adam_optimizer.minimize(loss, Q_network.variables,tape = None)
...
def main():
    ...
    try:
        ...
        for i in tqdm(range(samp_nb)):
            ...
            indices = tf.where(Ts[u][x]!=0)
            xp_prob = tf.gather(Ts[u][x], indices)
            xp_prob = tf.reshape(tf.constant(xp_prob), (xp_prob.shape[1],xp_prob.shape[0]))
            samples = tf.random.categorical(tf.math.log(xp_prob),num_samples = 1)
            xp = tf.gather(indices, samples).numpy().item()
    ...
    input = tf.keras.Input(shape=[3], name='state and action')
    layer1 = tf.keras.layers.Dense(64, activation='sigmoid')(input)
    layer2 = tf.keras.layers.Dense(64, activation='sigmoid')(layer1)
    layer3 = tf.keras.layers.Dense(1)(layer2)
    model = tf.keras.Model(inputs=input, outputs=layer3)
    Q_network = model
